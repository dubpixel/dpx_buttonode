...
21. **Raspberry Pi OS's `userconfig.service` (from `userconf-pi`) interactively prompts "Which user would you like to rename:" on `/dev/tty8` on every boot, unless `/boot/firmware/userconf.txt` is pre-seeded — Armbian has no equivalent.** Raspberry Pi OS ships a placeholder UID-1000 user by default; this wizard exists to let Imager-flashed users rename/password it on first boot. Confirmed live 2026-08-29 on real Pi 4 hardware — same class of problem as `dpx_openPanel` issue #25 (a locked placeholder account breaking headless setups). This project's security model is root-only SSH with no baked-in credential (gotcha 10a) — we don't want a second non-root account/wizard at all, so `dpx-buttonode.pkr.hcl` masks `userconfig.service` outright (`systemctl mask userconfig.service || true`) rather than faking a `userconf.txt`. Don't "fix" a hung/prompting first boot by writing a `userconf.txt` — check the mask survived instead.
22. **Raspberry Pi OS ships cloud-init by default — it visibly stalls first boot at "Reached target Cloud-init target" while searching for a datasource that will never exist.** Confirmed live 2026-08-29 on real Pi 4 hardware, right after fixing gotcha #21 — a second, independent first-boot delay in the same family. cloud-init is the actual mechanism behind Raspberry Pi Imager's "advanced options" customization on current releases, but this project's own first-boot provisioning (`dpx-set-hostname.service`, `dpx-init-ssh.service`, etc.) already covers everything cloud-init would otherwise do, so `dpx-buttonode.pkr.hcl` disables it via its documented marker file (`touch /etc/cloud/cloud-init.disabled`) plus masking `cloud-init.service`/`cloud-init-local.service`/`cloud-config.service`/`cloud-final.service`/`cloud-init.target` directly. If a Pi image still stalls at a cloud-init target after a build with this fix, check the marker/masks actually landed rather than assuming it's a different problem.
23. **`apt-get install xserver-xorg ...` without `--no-install-recommends` can silently pull in a default display manager, whose postinst flips the system's default systemd target from `multi-user.target` to `graphical.target` and enables itself.** Confirmed live 2026-08-29 on real Pi 4 hardware — a Lite-variant image (no desktop) reached `graphical.target` on boot, which should never happen, and every service including `dpx-buttonode-ui` was unreachable behind it (all ports closed, not just refused-while-still-starting). `install-dashboard.sh`'s X11/openbox dependency install now uses `--no-install-recommends` and explicitly runs `systemctl set-default multi-user.target` as a second layer regardless of what any package's postinst did — Dashboard launches its own X session directly via `xinit` in `dpx-dashboard.service`, it was never relying on the system's default target being graphical in the first place. If a future apt-get install is added to any provisioning script, default to `--no-install-recommends` unless there's a specific reason not to.
24. **`apt-get install`-triggered `update-initramfs` writes CAN silently corrupt the FAT32 boot partition on Raspberry Pi OS builds, in principle — a real, documented failure in the sibling `dpx_openPanel` project (same Packer `arm-image` + qemu-chroot toolchain).** ⚠️ **Correction: this was NOT actually the cause of this project's own 2026-08-29/30 Pi 4 boot stalls** — grepping the full build logs for "initramfs"/"initrd" turned up zero hits, meaning none of this project's own packages ever triggered it. The stub (`/usr/sbin/update-initramfs` → no-op, first provisioner step) is still in `dpx-buttonode.pkr.hcl` as a cheap defensive measure (matches `pi-gen`'s own approach; this project doesn't need an initramfs for SD boot), but it did not fix anything here — the real cause was gotcha #25. Lesson (see `[[feedback-hardware-debug-rigor]]` if referencing session memory): a bug from a sibling project using the same toolchain is a lead worth checking, not a conclusion to assert — verify against this project's own logs before treating it as the answer.
25. **THE ACTUAL cause of every Pi 4 first-boot web-UI-unreachable symptom this session: Python 3.13 (Raspberry Pi OS Trixie) fully removed the stdlib `crypt` and `spwd` modules (PEP 594) — `dpx-buttonode-ui.py`'s `import crypt` at module load meant the service was silently crash-looping (`ModuleNotFoundError`) on every single Raspberry Pi OS build all session, on every boot, forever.** Armbian's older Ubuntu-based Python still has both modules, which is exactly why this never surfaced on any Armbian board and only ever hit Raspberry Pi OS. Every other fix this session (gotchas 21-24 — userconfig.service, cloud-init, display-manager/graphical.target, update-initramfs) was real and worth keeping, but **none of them were why the web UI itself never came up** — that was this, the entire time, hiding underneath every boot-target red herring. Confirmed directly via `journalctl`/`systemctl status` over real SSH access to the device (see gotcha for how — a throwaway debug-SSH branch with a non-root sudo user, since root SSH login on stock Raspberry Pi OS has its own separate unresolved friction, not worth chasing further once a working non-root login was available). Fixed in `verify_root_password()`/`_crypt()`/`_get_shadow_hash()`: a direct `ctypes` binding to the system's real `libcrypt` (still pure stdlib, no pip dependency — a hand-rolled hash algorithm wasn't an option since Debian/Raspberry Pi OS default to yescrypt, not sha512crypt, so only the real libcrypt verifies correctly regardless of scheme), plus a direct `/etc/shadow` read in place of `spwd`. **If `dpx-buttonode-ui.service` ever won't start on a future Python bump (Armbian eventually moves to 3.13+ too), check `journalctl -u dpx-buttonode-ui` for `ModuleNotFoundError` before assuming a boot-sequence problem — this class of bug looks identical to a hung/stalled boot from the outside (port refused, nothing else diagnostic without console/SSH access) but is actually an instant crash-loop, not a stall.**
26. **OPEN / UNRESOLVED: a genuinely fresh rockpi-s flash (confirmed via the footer's `main@8080ce3`, matched against that exact build's own log) showed SSH as "enabled" ~2 hours after first boot, with no confirmed explanation.** What's been ruled out, with evidence, not assumption: (a) the shipped image — the build log shows `ssh.socket`'s enable symlinks explicitly removed at build time, nothing re-enables them anywhere later in the same build; (b) any first-boot/runtime script in this repo — `grep -rn "enable.*ssh" scripts/*.sh` matches only `dpx-init-ssh.service` (which just generates the password file, not the daemon); (c) an unauthenticated toggle — `do_POST`'s `/ssh` handler requires `verify_root_password(current_password)` to succeed before dispatching *any* action including `enable`, confirmed by reading the code directly, so this wasn't a bare unauthenticated request either. The user directly disputed the most likely-seeming explanation (leftover state from this session's own earlier SSH testing on the same physical unit) — so treat that as unconfirmed, not the answer. If this recurs on a genuinely fresh flash, the next step is capturing `journalctl` around the moment SSH actually flips on (needs a device with SSH/console access built in from first boot, e.g. the debug-SSH branch pattern from gotcha #25, to catch it live) rather than reasoning about it after the fact.
