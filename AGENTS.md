...
24. **`apt-get install`-triggered `update-initramfs` writes can silently corrupt the FAT32 boot partition on Raspberry Pi OS builds — the image still builds successfully and passes `fsck.fat`/MBR checks, but real hardware can't boot it, with a different-looking symptom each time.** Found by comparing against the sibling `dpx_openPanel` project, which uses the identical Packer `arm-image` + qemu-chroot toolchain and hit (and already documented, and fixed) this exact failure: a package's postinst triggers dpkg's initramfs-tools hook, which writes a ~50MB `update-initramfs` result into the small FAT32 boot partition via QEMU-emulated I/O inside the chroot — corrupting it in a way that only manifests as a bootloader failure on *real* hardware, invisible to every build-time/image-integrity check. This is the suspected real root cause behind the string of inconsistent Pi 4 first-boot stalls this project hit 2026-08-29/30 (different target each time — cloud-init, graphical.target, multi-user.target with SSH also unreachable) that individual targeted fixes (gotchas 21-23) didn't fully resolve. Fix (matching `dpx_openPanel` and `pi-gen`, the tool Raspberry Pi OS itself is built with): stub `/usr/sbin/update-initramfs` as a no-op, as the very first provisioner step, before any `apt-get install` runs anywhere in the build. This project doesn't need or use an initramfs for normal SD boot. If a future Pi 4/5 image still won't boot on real hardware despite a clean build, check this stub landed before assuming it's a new problem.
